...
function StatsTV() {
  ...
  const { postsTV } = useContext(PostContext); // Get posts from PostContext

  // Find the specific post by its ID (index)
  const post = postsTV[postId];
      ...
      <div class="row g-0">
        ...
        <div class="col-md-8">
          <div class="card-body">
            ...
            <br></br>
            <small class="text-body-secondary">Language:{post.lang}</small>
            <p class="card-text">{responseData["overview"]}</p>
            {/* <p class="card-text">
              Popularity:{responseData["popularity"]} <RiseorFallIcon />
            </p> */}
          </div>
        </div>
      </div>
